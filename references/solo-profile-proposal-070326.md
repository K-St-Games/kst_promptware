...
> **Status:** Draft for review — promotes to `profiles/` on ratification. **Extended by [`planning-cascade-proposal-070326.md`](planning-cascade-proposal-070326.md) (v2.1)**, which adds the vision→phase→weekly→spec planning layer for product repos (with a trigger-gated optional state card); this v1 remains the floor for small tools and experiments.
